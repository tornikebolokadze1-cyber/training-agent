...
# {prev_chunk_tail} is injected by transcribe_video() — the last ~500 chars of the
# previous chunk's transcript.  This concrete anchor prevents Gemini from echoing
# the prompt text or entering timestamp-repetition loops (the G3 L1 failure mode,
# confirmed 2026-05-12 on gemini-2.5-flash-lite with 30-min chunks).
TRANSCRIPTION_CONTINUATION_PROMPT = """შენ ხარ პროფესიონალი ტრანსკრიპტორი. ეს არის იგივე ლექციის გაგრძელება (ნაწილი {chunk_number}/{total_chunks}).

წინა ნაწილი მთავრდებოდა შემდეგნაირად:
---
{prev_chunk_tail}
---

გააგრძელე ტრანსკრიპტი ზემოთ ნაჩვენები ადგილიდან. მოუსმინე ამ ნაწილის აუდიოს და გადმოეცი მხოლოდ ის, რაც ამ ნაწილში ითქვა.

მოთხოვნები:
- გადმოეცი ყველაფერი ზუსტად, სიტყვა სიტყვით
- მონიშნე ვინ ლაპარაკობს (ლექტორი, მონაწილე, კითხვა აუდიტორიიდან)
- დროის მარკერები განაგრძე წინა ნაწილის ბოლო მარკერიდან [HH:MM] ფორმატით
- ტექნიკური ტერმინები დატოვე ინგლისურად

მნიშვნელოვანია: ნუ გაიმეორებ წინა ნაწილის ტექსტს — დაიწყე იქიდან, სადაც ეს ნაწილის აუდიო იწყება."""
